...
class DBEntity(@(ApiModelProperty @field)(hidden= true)val tableName: String) extends Serializable{
  ...
  private def getCreateSql() = {
    ...
    if (hasId) {
      val IdKVs = fieldKVs map (kv => if (kv._1 == "id") kv._1 -> (( if(kv._2.contains("INT")) kv._2 +" AUTO_INCREMENT " else "VARCHAR(32)")) else kv)
  ...
  def updateNoEmpty(where: String, fields: String*) = {
    val m = getFieldKeyValues(fields: _*) filter (v => v._2 != null && !DBEntity.isEmpty(v._2))
    val updates = (m map (_._1) toList)
    update(where, updates: _*)
  }
  ...
  def insertWithId(fields: String*) = {
    ...
    getClass.getMethods.filter(_.getName=="id").map(_.invoke(this)).headOption.getOrElse("")
  }
  ...
  def queryOne(sql: String, param: AnyRef*): Option[_ <: DBEntity] = {
  ...
  def quickById(id: String,onlyCache:Boolean=false): Option[_ <: DBEntity] = {
    val key = "dbcache_" + tableName
    val cache =Cache.getCache(key+id)
    if(cache.isEmpty &&onlyCache )
      None
    else if (cache.isEmpty|| cache.get.isInstanceOf[String]) {
      val data = queryById(id)
      if (!data.isEmpty) {
        Cache.setCache(key+id,data.get,3600)
      }
      data
    }else
      Some(cache.get.asInstanceOf[DBEntity])
  }

  def quickByIds(ids: List[String]): List[_ <: DBEntity] = {
    ids.map(quickById(_)).filter(_.isDefined).map(_.get)
  }
...
object DBEntity {
  ...
  def queryById[T](clazz: Class[_ <: T], tableName: String, id: String): Option[T] = {
  ...
  def quickById[T<:DBEntity](clazz: Class[_ <: T], tableName: String,id: String): Option[T] = {
    val key = "dbcache_" + tableName
    val cache =Cache.getCache(key+id)
    if (cache.isEmpty||cache.get.isInstanceOf[String]) {
      val data = queryById(clazz,tableName,id)
      if (!data.isEmpty) {
        Cache.setCache(key+id,data.get,3600)
      }
      data
    }else
      Some(cache.get.asInstanceOf[T])
  }
  ...
  def sql(sql: String, params: AnyRef*) = {
    DBTool.update(sql, params: _*)
  }
...
private object DBTool {
  ...
  def update(sql: String, params: AnyRef*): Int = {
    val conn = getConn()
  ...
  private def  setStatem(st: PreparedStatement,index:Int,obj:Any)={
    if(obj.isInstanceOf[String]){
      st.setString(index,obj.asInstanceOf[String])
    }else if(obj.isInstanceOf[Boolean]){
      st.setBoolean(index,obj.asInstanceOf[Boolean])
    }else if(obj.isInstanceOf[Int]){
      st.setInt(index,obj.asInstanceOf[Int])
    }else if(obj.isInstanceOf[Float]){
      st.setFloat(index,obj.asInstanceOf[Float])
    }else if(obj.isInstanceOf[Long]){
      st.setLong(index,obj.asInstanceOf[Long])
    }else if(obj.isInstanceOf[Double]){
      st.setDouble(index,obj.asInstanceOf[Double])
    }else if(obj.isInstanceOf[BigDecimal]){
      st.setBigDecimal(index,obj.asInstanceOf[BigDecimal].bigDecimal)
    }else if(obj.isInstanceOf[Date]){
      st.setDate(index,obj.asInstanceOf[Date].toSqlDate)
    }else {
      st.setString(index,obj.toString)
    }
  }
