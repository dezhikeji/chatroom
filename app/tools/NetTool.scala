...
import org.apache.http.conn.socket.{ConnectionSocketFactory, LayeredConnectionSocketFactory, PlainConnectionSocketFactory}
...
import org.apache.http.{HttpEntity, HttpHost, config}
import org.apache.http.util.EntityUtils
import common.Tool._
import java.security.cert.X509Certificate
import javax.net.ssl.{SSLContext, TrustManager, X509TrustManager}

import org.apache.http.conn.ssl.{NoopHostnameVerifier, SSLConnectionSocketFactory, SSLSocketFactory}
 ...
 *
  ...
  def wrapClient(base:CloseableHttpClient):CloseableHttpClient=
  {
    try {
      ...
      val sslSF = new SSLConnectionSocketFactory(ctx, NoopHostnameVerifier.INSTANCE)
  ...
  def realUrl(url:String):String={
    val v=HttpClientBuilder.create().setDefaultRequestConfig(RequestConfig.custom().setRedirectsEnabled(false).build()).build().execute(new HttpGet(url))
    if(v.getAllHeaders.filter(_.getName.toLowerCase.contains("location")).size>0){
      realUrl(v.getAllHeaders.filter(_.getName.toLowerCase.contains("location")).head.getValue)
    }else{
      url
    }
  }
