...
object Tool {
  ...
  private val settingObjectCache = new mutable.HashMap[String, String]()
  ...
  def sha1(str: String): String = {
    ...
    val sb: StringBuilder = new StringBuilder
    for (b <- sha.digest(str.getBytes("utf-8"))) {
      sb.append(str2HexStr(b))
    }
    return sb.toString
  }
  ...
  def getExpr(context: String, startseparate: String,
    ...
    if (null == context || context.isEmpty || context.indexOf(startseparate) == -1) {
      ...
      val end = if (endSeparate == null) context.length - 1 else context.indexOf(endSeparate, start + 1)
  ...
  def getSettingMap(): Map[String, String] = {
    settingObjectCache.toMap[String, String]
  }

  def setSetting(data: Map[String, String]) = {
    settingObjectCache.clear()
    data.foreach(v => settingObjectCache.put(v._1, v._2))
  }

  def cacheMethod[T <: AnyRef](key: String, time: Int)(f: => T): T = {
  ...
  def cacheStaticMethod[T <: AnyRef](key: String, time: Int)(f: => T): T = {
    val value = Cache.getStaticCache(key)
    if (value.isDefined) {
      value.get.asInstanceOf[T]
    } else {
      val v = f
      run {
        Cache.setStaticCache(key, v, time)
      }
      v
    }
  }
  ...
  implicit class StringAddMethod[A <: String](bean: A) {
    ...
    def firstLowerCase = if (null == bean || bean.isEmpty) bean else (bean.head.toString.toLowerCase + bean.drop(1))
